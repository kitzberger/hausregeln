# Hermann Hermann

## Deck

Verwendet wird ein doppeltes Skatkarten-Set.

## Ziel des Spiel

Alle Karten loswerden und nicht wieder ins Spiel hineingezogen werden.

## Ablauf

Wie Mau-Mau, aber mit zweiter Ebene: Hermann-Hermann.

Jede Dame triggert den Hermann-Hermann-Modus. Eine weitere Dame beendet ihn.

In Hermann-Hermann-Modus:

* muss "Hermann" bzw. "Hermann-Hermann" statt "Mau" und "Mau-Mau" gesagt werden,
* gelten die Mau-Mau Ablageregeln nicht,
* müssen einfach abwechselnd rote und schwarze Karten gespielt werden,
* darf keine Frage gestellt werden,
* darf nicht geflucht werden,
* muss auf eine gespielte 10 jeder Spieler die Hand darauf klatschen, man darf nicht der letzte sein.

Bei jeglichem Regelverstoß muss eine Karte gezogen werden. So kann auch ein eigentlich bereits "fertiger" Spieler wieder ins Spiel kommen.

Das 10er-Klatschen darf natürlich auch angetäuscht werden. Den letzten den man damit hinters Licht führt, der muss nehmen. Falls niemand so doof ist, muss man selbst nehmen.

## Spezialregel Mau-Mau-Modus

Im Mau-Mau-Modus darf man Karten einwerfen, falls man die gleiche hat. Dies gilt nicht für den Hermann-Hermann-Modus, weil dort ja die Ablageregel rot/schwarz gilt.
